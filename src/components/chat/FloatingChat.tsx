...
export default function FloatingChat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "ai", text: "¡Hola! Dime qué gastaste, a quién le prestaste dinero o cuánto ahorraste hoy." }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await processChatMessage(userText);
      setMessages(prev => [...prev, { role: "ai", text: response.reply }]);
    } catch (error) {
      setMessages(prev => [...prev, { role: "ai", text: "Uy, algo falló en mi circuito. Intenta de nuevo." }]);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-800">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'ai' && <div className="w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center shrink-0"><Bot size={16} /></div>}
                
                <div className={`p-3 rounded-2xl max-w-[75%] text-sm ${msg.role === 'user' ? 'bg-gray-700 text-white rounded-tr-sm' : 'bg-gray-700 border border-gray-600 text-gray-200 rounded-tl-sm'}`}>
                  {msg.text}
                </div>

                {msg.role === 'user' && <div className="w-8 h-8 bg-gray-500 text-gray-200 rounded-full flex items-center justify-center shrink-0"><User size={16} /></div>}
              </div>
            ))}
            {isLoading && (
              <div className="flex gap-2 justify-start">
                <div className="w-8 h-8 bg-blue-600 text-white rounded-full flex items-center justify-center"><Bot size={16} /></div>
