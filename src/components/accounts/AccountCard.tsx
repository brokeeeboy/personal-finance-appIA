...
import { CreditCard, Landmark, Wallet, Trash2, Pencil } from "lucide-react";
...
export default function AccountCard({ account }: { account: Account }) {
  ...
  const getIcon = () => {
    ...
    if (account.type === "CHECKING")
      return <Landmark className="text-indigo-600" />;
  ...
  const handleDelete = () => {
    if (
      confirm(
        `¿Estás seguro de eliminar la cuenta ${account.name}? Se borrarán todas sus transacciones.`,
      )
    ) {
      startTransition(() => {
      ...
      <div className="mt-6">
        ...
        {account.type === "CREDIT" && (
          <div className="mt-3 space-y-2">
            ...
            {(account.billingDay || account.paymentDay) && (
              <div className="rounded-lg bg-slate-50 border border-slate-200 p-2 text-xs text-slate-600">
                <p>
                  <span className="font-semibold">Facturación:</span> día{" "}
                  {account.billingDay ?? "-"}
                </p>
                <p>
                  <span className="font-semibold">Pago:</span> día{" "}
                  {account.paymentDay ?? "-"}
                </p>
              </div>
            )}
