...
export async function saveImportedTransactions(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const balanceChange = sanitizedTransactions.reduce(
      (total, t) => total.plus(computeBalanceChange(account.type, t.type, t.amount)),
      new Prisma.Decimal(0),
    );
    await applyBalanceChange(tx, accountId, balanceChange);
  });
