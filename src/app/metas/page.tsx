      ...
      <main className="flex-1 md:ml-64 p-6 md:p-8 pb-24 md:pb-8">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-500">Planifica tus próximos grandes pasos financieros.</p>
          </div>
          <div className="flex items-center gap-4">
            {totalGoals > 0 && (
              <div className="flex items-center gap-2 bg-yellow-100 text-yellow-700 px-4 py-2 rounded-lg font-medium text-sm">
                <Trophy size={18} />
                <span>{completedGoals} de {totalGoals} logradas</span>
              </div>
            )}
            <NewGoalForm />
          </div>
        ...
        {goals.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-300">
            <p className="text-gray-500 mb-4">Aún no has definido ninguna meta.</p>
          </div>
        ) : (
